...
function makeMediaWikiRedactorDeserializer(redacted_pages) {
    ...
    return (kafkaMessage) => {
        ...
        if ( // case mediawiki.page_change.v1
            ...
            redacted_pages[km.message.wiki_id].includes(km.message.page.page_title.replace('_', ' '))
        ) {
            ...
            // Page titles may or may not use underscores instead of spaces
            redacted_pages[km.message.database].includes(km.message.page_title.replace('_', ' '))
        ) {
