...
function makeMediaWikiRedactorDeserializer(redacted_pages) {
    return (kafkaMessage) => {
        ...
        if ( // case mediawiki.page_change.v1
            ...
            km.message.wiki_id === 'ruwiki' &&
            redacted_pages.includes(km.message.page.page_title)
        ) {
            ...
            km.message.wiki === 'ruwiki' &&
            redacted_pages.includes(km.message.title)
        ) {
            delete km.message.user;
        } else if ( // case all other streams
            km.message.database === 'ruwiki' &&
            redacted_pages.includes(km.message.page_title)
        ) {
